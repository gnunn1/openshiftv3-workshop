...
and edit the values to use the following values:

....
Add to Project: consoleproject-userXX
...
In the command line, switch to the new project:

....
oc project consoleproject-userXX
....

While we are waiting we can add a label to the mysql deployment, this label
determines how the deployment is displayed with respect to applications in the OpenShift web console.
...
Add to Project: consoleproject-userXX (this should be the same project as MySQL Ephemeral)
...
....
$ oc project
Using project "consoleproject-userXX" on server "{{CONSOLE_ADDRESS}}".
