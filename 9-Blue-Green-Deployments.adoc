...
We will be using an environment variable in order to change the color of the box; but
...
Create a route by exposing the service as you have done in previous labs. *Remember* to substitute the userXX when you expose the service:
...
You can route traffic to both services thereby achieving blue-green deployments. Weighting traffic to primarily
one service versus the other achieves a canary deployment, this is also known as "canary in a coal mine".
